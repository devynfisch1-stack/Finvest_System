...
/* ================================================================== */
/* Live-Daten vom eigenen Backend (GitHub Actions -> data/stocks.json).
...
/* Übersetzt eine Zeile aus dem Backend (run.py -> assemble()) in die
   ...
   quality/valuation/dislocation/quality_sub/gate_ok, "m" statt "move" bei
   News-Ereignissen, usw.). */
function adaptBackendRow(row) {
  const qs = row.quality_sub || {};
  return {
    ticker: row.ticker, name: row.name, domain: row.domain, region: row.region,
    sector: row.sector, isFin: row.isFin, cap: row.marketCap,
    q: row.quality, v: row.valuation, disl: row.dislocation, overall: row.overall,
    status: row.status, signal: row.signal, confidence: row.confidence,
    qIn: {
      roic: qs.roic, roicTrend: qs.roic_trend, eq: qs.earnings_quality,
      marginStab: qs.margin_stability, balance: qs.balance, dilution: qs.dilution,
    },
    peNow: row.peNow, peHist: row.peHist, evNow: row.evNow, evHist: row.evHist,
    drawdown: row.drawdown, impliedGrowth: row.impliedGrowth, realistic: row.realistic,
    events: (row.events || []).map((e) => ({
      t: e.t, d: e.d, m: e.move != null ? e.move : e.m, type: e.type, s: e.s,
      sum: e.sum, bullets: e.bullets,
    })),
    verdict: typeof row.verdict === "string" ? { txt: row.verdict, warn: false, good: row.signal } : row.verdict,
    gate: row.gate_ok !== undefined ? row.gate_ok : true,
    report: row.report,
  };
}
...
export default function App() {
  ...
  const [liveData, setLiveData] = useState(null);       // {US:[...], CH:[...]} echte Backend-Daten
  ...
  useEffect(() => {
    ...
    fetch(LIVE_DATA_URL, { cache: "no-store" })
      ...
      .then((json) => {
        if (cancelled || !json || !json.regions) throw new Error("Unerwartetes Format");
        const adapted = {
          US: (json.regions.US || []).map(adaptBackendRow),
          CH: (json.regions.CH || []).map(adaptBackendRow),
        };
        setLiveData(adapted);
        setLiveStatus("ok");
      })
  ...
  /* Aktives Universum: echte Daten, wo vorhanden; sonst Beispieldaten je
     Ticker als Fallback (z. B. falls ein Titel im letzten Lauf fehlgeschlagen ist). */
  const effectiveUniverse = useMemo(() => {
    if (!liveData) return UNIVERSE;
    const merge = (region) => {
      const liveByTicker = new Map(liveData[region].map((s) => [s.ticker, s]));
      return UNIVERSE[region].map((fallback) => liveByTicker.get(fallback.ticker) || fallback)
        .sort((a, b) => (b.cap || 0) - (a.cap || 0));
    };
    return { US: merge("US"), CH: merge("CH") };
  }, [liveData]);

  const list = useMemo(() => {
    const q = query.trim().toLowerCase(), base = effectiveUniverse[region];
    return q ? base.filter((s) => s.name.toLowerCase().includes(q) || s.ticker.toLowerCase().includes(q)) : base;
  }, [region, query, effectiveUniverse]);
  const stock = selected ? effectiveUniverse[selected.region].find((s) => s.ticker === selected.ticker) : null;
  ...
  const archiveStock = archivePick ? effectiveUniverse[archivePick.region].find((s) => s.ticker === archivePick.ticker) : null;
  ...
  return (
    <div style={S.root}>
      ...
      {archiveOpen ? (
        ...
        ? <ListView {...{ list, region, setRegion, query, setQuery, onPick: setSelected, stamp, favorites, toggleFavorite, liveStatus }} />
...
function ListView({ list, region, setRegion, query, setQuery, onPick, stamp, favorites, toggleFavorite, liveStatus }) {
  ...
  return (
    <div style={S.body}>
      ...
      <div style={S.stampRow}>Letzter Durchlauf {run} · {
        liveStatus === "ok" ? <span style={{ color: GREEN }}>Echte Daten geladen</span>
        : liveStatus === "error" ? <span style={{ color: "#8b8d92" }}>Beispieldaten (kein Live-Datenabruf gefunden)</span>
        : <span style={{ color: "#8b8d92" }}>Lade echte Daten…</span>
      }</div>
